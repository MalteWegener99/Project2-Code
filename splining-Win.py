...
import scipy
from stefiando.interpolation import interp
...
def analyse(file_name):
    ...
    xp, yp, cubic = interp(positions[:,1], positions[:,0], strain[:,0,-1], shape, algorithm='cubic', extrapolate=False)
    ...
    def animate(t):
        ...
        ax.set_title(date)
        a,b,th = interp(positions[:,1], positions[:,0],  strain[:,0,t], shape, algorithm='cubic', extrapolate=False)
