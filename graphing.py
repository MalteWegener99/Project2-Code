...
def parse_binary_llh(path):
    ...
    with open(path, 'rb') as file:
        ...
        axarr[0].plot([mindate, times[-1]], [north[1], north[1] + north[0]*times[-1]])
        axarr[1].plot([mindate, times[-1]], [east[1], east[1] + east[0]*times[-1]])
        axarr[2].plot(times, list(map(lambda x: to_fit(x, up[0], up[1], up[2], up[3]), times)))
